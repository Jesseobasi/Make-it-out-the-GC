          ...
          <header className="panel overflow-hidden p-4 sm:p-6">
            <div className="flex flex-col gap-4">
              <div className="flex items-start justify-between gap-3">
                <Link
                  to="/"
                  className="inline-flex items-center gap-2 rounded-full bg-slate-800 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400"
                >
                  Best Day Scheduler
                </Link>
                <div className="flex items-center gap-2">
                  {isAuthenticated ? (
                    <>
                      <Link to="/my-events" className="rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold text-slate-300">
                        My events
                      </Link>
                      <button onClick={signOut} className="rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold text-slate-300">
                        Logout
                      </button>
                    </>
                  ) : authAvailable ? (
                    <Link to="/login" className="rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold text-slate-300">
                      Login
                    </Link>
                  ) : null}
                  <button
                  ...
                  <span className="rounded-full bg-indigo-900/30 px-3 py-1 text-xs font-semibold text-indigo-400">
                    {eyebrow}
                  </span>
                </div>
              </div>
